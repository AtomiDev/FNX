...
<?php
    ...
    $missingData=empty($_POST); //The empty() function is used to check whether a variable is empty or not. FALSE if var_name has a non-empty and non-zero value.
    
    date_default_timezone_set('Asia/Jerusalem'); //Sets the default timezone used by all date/time functions in a script

    ini_set("log_errors", 1);//Sets error log to TRUE

    $logFileName = './'.basename(__FILE__, '.php').'.log';// Sets the log file name and path

    ini_set("error_log", $logFileName);//Tells logger where logfile is
...
if(!$missingData){
  //  echo("missing data");
   if($debug) error_log("generating response");
    ...
    $LeadTypeID = '37'; //LEADWIZE type ID
    $CampaignID = '418';// LEADWIZE  campaign ID
    //TBD
    $ProviderLeadPK = '0';
